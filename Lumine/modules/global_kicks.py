...
from Lumine import Message, Update, Bot, User, Chat, ParseMode
from Lumine.error import BadRequest, TelegramError
from Lumine.ext import run_async, CommandHandler, MessageHandler, Filters
from Lumine.utils.helpers import mention_html

from telegram.ext import CallbackContext
from Lumine import dispatcher, OWNER_ID, SUDO_USERS, SUPPORT_USERS
...
def gkick(update: Update, context: CallbackContext):
    ...
    send_to_list(bot, SUDO_USERS + SUPPORT_USERS,
                 ...
                 "<b>Kicked User:</b> {mention_html(user_chat.id, user_chat.first_name)}\n" \
                 "<b>Kicked User ID:</b> <code>{user_chat.id}</code>\n" \
                 "<b>Event Stamp:</b> <code>{current_time}</code>".format(mention_html(kicker.id, kicker.first_name),
